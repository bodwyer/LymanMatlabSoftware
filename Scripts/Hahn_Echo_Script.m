...
N_REPS = 200; % Number of times to repeat whole process (all Tau)
TAU_MIN = 20; % ns
TAU_MAX = 50e3; % ns
N_POINTS = 51;
...
peakPredict = 1/(((10.705/10000)*(2.87e3 - FREQ/1e3)/(2.8)*1e-3));
tauStep = 2.5*double(idivide(round(peakPredict*10), int32(25)));
% Create vector of free precession times and vectors to hold data
% tauStepEx = round((TAU_MAX - TAU_MIN)/N_POINTS);
% tauStep = 2.5*double(idivide(tauStepEx*10, int32(25)));
%tauStep = 3700;
tauVec = [TAU_MIN, tauStep:tauStep:TAU_MAX];
% tau2 = 280:tauStep:TAU_MAX;
% tau3 = 290:tauStep:TAU_MAX;
% tau4 = 300:tauStep:TAU_MAX;
% tauVec = sort([tau1, tau2, tau3, tau4]);
...
for i = 1:N_REPS
    ...
    for it = 1:length(tauVec)
        ...
        timePerExp = (20 + 3*IQ_WIDTH + 3*PI_TIME +  WAIT_TIME + 2*DELAY + 2*initTime + 2*readoutTime + referenceTime)*1e-9;
        timeNotTau = (N_REPS - i + 1)*N_EXPERIMENTS*(timePerExp)*length(tauVec);
        tauTime = 2*1e-9*N_EXPERIMENTS*((N_REPS - i)*sum((tauVec)) + sum(tauVec(it:end)));
...
saveData = {tauVec, sigVec, refVec, refVecDark, countsNorm, transform};
